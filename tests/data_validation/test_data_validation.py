...
def test_no_missing_values(load_sample_data):
    ...
    ), "❌ Data contains missing values!"  # nosec B101
...
def test_no_duplicates(load_sample_data):
    ...
    assert (
        duplicate_count == 0
    ), f"❌ Found {duplicate_count} duplicate rows!"  # nosec B101
...
def test_expected_columns(load_sample_data):
    ...
    ), "❌ Schema mismatch in columns!"  # nosec B101
...
def test_data_types(load_sample_data):
    ...
    assert pd.api.types.is_numeric_dtype(
        df["age"]
    ), "❌ Age column should be numeric!"  # nosec B101
